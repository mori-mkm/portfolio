...
5. M1-09 candidates: Footer (last remaining Home section, PORTFOLIO_SPEC §21/WIREFRAME §31 - not yet built), Contact production integration (item 3 above), or deploy readiness (Vercel env vars, ADR-006) - not yet decided which comes first; ask before assuming.
...
- ADR-016: Application Job removed from Selected Work entirely (not reassigned, not replaced) - Selected Work is now Procurement Intelligence + Employee Attrition Prediction only.
