...
M1 - Home foundation (M1-02 through M1-05 done; M1-06 not started)
...
- None. M1-05 is done; M1-06 (Research & Recognition content allocation) has not been started.

## Next planned

1. Confirm the Resume Google Doc's sharing is "Viewer" (not "Editor") - see DECISIONS/CURRENT_TASK note.
2. Decide what to do with remaining unallocated content facts (Santander Hackathon 1st place, additional GitHub repos: closer-ai, UFSCar, retail-sales-forecasting) - not yet incorporated into any Home section or PROJECT_CONTENT.md. Do not build this out silently - scope it as its own task first, and per ADR-012 check it isn't already featured elsewhere on Home before adding it anywhere. (The CNN research finding and the four-company Experience timeline ARE now allocated - M1-04/M1-05 above - do not re-add either elsewhere.)
3. M1-06 (Research & Recognition content allocation) needs a content-allocation decision pass BEFORE implementation - the old Research wireframe (HOME_WIREFRAME §24-25) predates ADR-012 and its candidate topics (e.g. "Economic Index Construction" associated with Steel Indicator) may overlap content already placed in Case Studies. Decide what's unique to Research first, then build. Do not implement M1-06 yet.
4. Move M1-06 into `CURRENT_TASK.md` when ready to start it - not yet started.
