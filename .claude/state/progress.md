...
4. Fix the pre-existing `Header.tsx` 768px horizontal overflow (see Known pitfalls) - not yet scheduled to any milestone.
5. Move M1-08 into `CURRENT_TASK.md` when ready to start it - not yet started.
...
- **`Header.tsx` has an 18px horizontal overflow at exactly 768px viewport width** (found during M1-07 visual verification, confirmed via DOM inspection that the overflowing element is `<HEADER>` itself, not any other section). Pre-existing, not caused by any M1-03 through M1-07 change (all confirmed via `git diff --stat` to leave `Header.tsx` untouched). Not yet fixed - needs its own task. Likely cause: desktop nav (logo + 7 nav items + language switcher + Resume button) doesn't quite fit at 768px, just before/around whatever breakpoint triggers the mobile "MENU" toggle.
